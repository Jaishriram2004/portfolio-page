...
const CatchGame: React.FC<Props> = ({ onGameComplete, gameDuration = 5 }) => {
  ...
  const handleMouseMove = useCallback((e: MouseEvent) => {
    ...
    const startGame = () => {
        ...
        setItemPosition({ x: GAME_AREA_WIDTH / 2 - ITEM_SIZE / 2, y: 0 });
    };

  useEffect(() => {
    if (!isGameActive || showStartScreen) return;
  ...
  }, [isGameActive, onGameComplete, gameDuration, catcherPosition, showStartScreen]);
  ...
  useEffect(() => {
    ...
    if (showStartScreen) {
        return (
            <div className="game-card flex flex-col items-center justify-center p-6 bg-gray-800 rounded-lg shadow-lg" style={{ width: GAME_AREA_WIDTH, height: GAME_AREA_HEIGHT + 80 }}>
                <h3 className="text-2xl font-bold text-white mb-4">Play to reveal</h3>
                <p className="text-gray-300 mb-6">Catch the item before time runs out!</p>
                <button
                    onClick={startGame}
                    className="px-6 py-3 font-bold text-white bg-indigo-600 rounded-lg shadow-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-50 transition-transform transform hover:scale-105"
                >
                    Start Game
                </button>
            </div>
      ...
      >
          ...
          <>
            ...
            <div
              ...
              style={{
                ...
                width: CATCHER_WIDTH,
                height: ITEM_SIZE / 2,
                backgroundColor: 'oklch(25% 0.02 240)',
        ...
        {!isGameActive && (
          <p className="game-message">
            {timeLeft === 0 ? "Time's up!" : (itemPosition.y + ITEM_SIZE >= GAME_AREA_HEIGHT ? "You won!" : "You won!")}
          </p>
        )}
      </div>
    </div>
